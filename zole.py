...
def train(dataloader, model, optimizer, log, epoch=0):
    ...
    for batch_idx, (imgL, imgR, disp_L) in enumerate(dataloader):
        ...
        imgL = imgL[:,:,randomH:(randomH+h),randomW:(randomW+w)]
        imgR = imgR[:, :, randomH:(randomH + h), randomW:(randomW + w)]
        disp_L = disp_L[:, randomH:(randomH + h), randomW:(randomW + w)]


        imgL = imgL.float().cuda()
        imgR = imgR.float().cuda()
        disp_L = disp_L.float().cuda()
        output0 = model(imgL, imgR)[3]

        imgL = F.interpolate(imgL, scale_factor= 1.5, mode='bilinear', align_corners=False)
        imgR = F.interpolate(imgR, scale_factor= 1.5, mode='bilinear', align_corners=False)
        #disp_L = F.interpolate(disp_L[:,None,:], scale_factor=1 / 1.5, mode='bilinear', align_corners=False)
        #disp_L = disp_L * 1.5
        output1 = model(imgL, imgR)[3]
        output1 = F.interpolate(output1[:,None,:], scale_factor=1/1.5, mode='bilinear', align_corners=False)[:,0,:]
        output1 = output1/1.5


        loss0 = GERF_loss(disp_L, output0, args)
        loss1 = GERF_loss(disp_L, output1, args)
        ...
        if(False):
            ...
            disp_TRUE_ = output0.cpu().detach().numpy()[0]
            disp_NET_ = output1.cpu().detach().numpy()[0]
